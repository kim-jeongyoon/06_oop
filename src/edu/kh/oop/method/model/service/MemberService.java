...
public class MemberService { // 클래스
	...
	*/
	...
	public String selectMember() {
		...
		// 2) 로그인이 되어있는 경우 회원정조를 출력할 문자열을 만들어서 반환(return) 
		// 단, 비밀번호는 제외
		
		if(loginMember == null) { // 회원가입을 먼저 안한 경우
			return "로그인 후 이용해주세요.";
	...
	public int updateMember() {
		...
		// 1) 로그인 여부 판멸, 로그인이 되어있지 않으면 -1반환
		if(loginMember == null) {
			return -1;
		}
		// 2) 수정할 회원 정보 입력받기 (이름 나이)
		...
		// sc.nextLine(); 입력버퍼 정리하기, 이걸 안하면  
		
		sc.nextLine();
		
		// 3) 비밀번호 입력받아서 로그인한 회원의 비밀번호와 일치한지 확인
		System.out.print("비밀번호 입력 : ");
		String inputPw = sc.next();
		// 4) 비밀번호가 같은 경우: 로그인한 회원의 이름, 나이 정보를 입력 받은 값으로 변경 후 1반환
